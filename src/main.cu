...
int main(int argc, char *argv[]) {
    ...
    HLOG("GPU max clock rate: %.0f MHz (%0.2f GHz)", dev_prop.clockRate * 1e-3f, dev_prop.clockRate * 1e-6f);
    HLOG("Memory clock rate: %.0f MHz (%0.2f GHz)", dev_prop.memoryClockRate * 1e-3f, dev_prop.memoryClockRate * 1e-6f);
    HLOG("Memory bus width: %d-bit", dev_prop.memoryBusWidth);
    HLOG("Total amount of global memory: %.0f MBytes (%llu Bytes)",
    ...
    HLOG("L2 cache size: %.0f KBytes (%d Bytes)", static_cast<float>(dev_prop.l2CacheSize / 1024.0f),
